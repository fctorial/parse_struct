...
public class MMap extends APersistentMap implements IObj, IEditableCollection, IMapIterable, IKVReduce {
    ...
    public MMap withMeta(IPersistentMap meta){
        if(meta() == meta)
            return this;
        return new MMap(meta, array);
    }

    MMap create(Object... init){
        return new MMap(meta(), init);
    }
    ...
    static public MMap createWithCheck(Object[] init){
        ...
        return new MMap(init);
    }

    static public MMap createAsIfByAssoc(Object[] init){
        if ((init.length & 1) == 1)
     ...
     */
    public MMap(Object[] init){
        this.array = init;
        this._meta = null;
    }


    public MMap(IPersistentMap meta, Object[] init){
        this._meta = meta;
        this.array = init;
    }
    ...
    public Iterator iterator(){
        return new Iter(array, MAKE_ENTRY);
    }

    public Iterator keyIterator(){
        return new Iter(array, MAKE_KEY);
    }

    public Iterator valIterator() {
        return new Iter(array, MAKE_VAL);
    }
